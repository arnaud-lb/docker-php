...
if [ -n "${BASHBREW_SCRIPTS:-}" ]; then
	...
	wget -O "$jqt" 'https://github.com/docker-library/bashbrew/raw/9f6a35772ac863a0241f147c820354e4008edf38/scripts/jq-template.awk'
fi
...
for version; do
	...
	for dir in "${variants[@]}"; do
		...
		cp -a \
			...
			docker-php-source \
			"$version/$dir/"
		if [ "$variant" = 'apache' ] || [ "$variant" = 'apache-zts' ]; then
